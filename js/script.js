...
function irregular_verbs_game(id){
	...
	this.main_structure = function(id){
		$("#"+id).empty();
		$("#"+id).append('<div class="row"><div class="col-sm-12 title_box"><h1 class="text-center text-uppercase">Verbele neregulate</h1></div></div>');	
		$("#"+id).append('<div class="quiz_verb_group_container"><div class="row"><div class="col-sm-12"><ul class="quiz_verb_group"><li>A, B</li><li>C, D</li><li>E, F</li><li>G, H</li><li>K, L</li><li>M - Q</li><li>R</li><li>S</li><li>T - W</li></ul></div></div></div>');	
	}
	
	this.structure_footer = function(id){
		if($('footer').length > 0){
			$('footer').remove();
		}
...
function quiz_game(id){
	...
	this.generate_group_quiz = function(elem){		
		...
		console.warn('generate_group_quiz', id, elem.text(), letters);
		
		verb_group = [];
		for(var i in letters){			
			for(var j in verbs){
				if(verbs[j].letter == letters[i]){
					verb_group.push(verbs[j]);
				}				
			}
		}
		
		console.warn('verb_group', verb_group);
	...
	this.verify_answer = function(nr){
		...
		console.warn('verify_answer', myform01, myform02, myform03, form01, form02, form03, form03.includes(myform03));
		...
		if(form01 != myform01 || form02 != myform02 || !form03.includes(myform03)){
			...
			console.warn('myanswer', myanswer);
